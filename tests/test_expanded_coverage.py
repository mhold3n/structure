...
class TestDisambiguatedRequests:
    ...
    def test_explicit_lbm_passes(self):
        ...
        request = TaskRequest(
            ...
            user_input="Convert 10 lbm to kg"
        )
        spec = classify_task(request)
    ...
    def test_explicit_lbf_passes(self):
        ...
        request = TaskRequest(
            request_id="disambig-3",
            user_input="A force of 100 lbf is applied"
        )
        spec = classify_task(request)
...
class TestGateOrdering:
    ...
    def test_schema_gate_always_included(self):
        ...
        request = TaskRequest(
            request_id="order-1",
            user_input="Simple math question: 2+2"
        )
        spec = classify_task(request)
    ...
    def test_unit_gate_when_units_present(self):
        ...
        request = TaskRequest(
            request_id="order-2",
            user_input="Convert 10 kg to lb"
        )
        spec = classify_task(request)
    ...
    def test_ambiguity_gate_when_terms_detected(self):
        ...
        request = TaskRequest(
            request_id="order-3",
            user_input="What is the specific weight?"
        )
        spec = classify_task(request)
...
class TestRegressionScenarios:
    ...
    def test_lb_vs_lbf_confusion(self):
        ...
        request = TaskRequest(
            request_id="regress-2",
            user_input="A weight of 100 lb"
        )
        spec = classify_task(request)
        gate_results = run_gates(spec)
        blocking = get_blocking_decisions(gate_results)
