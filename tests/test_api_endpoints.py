...
def test_experiment_endpoint(mock_orchestrator):
    ...
    with patch("gateway.main.classify_task") as mock_classify, \
...
def test_session_endpoints(mock_orchestrator):
    ...
    workflow = Workflow(
        ...
        name="Test Workflow",
        steps=[
            WorkflowStep(step_id="s1", description="Test step", status=WorkflowStatus.BLOCKED, spec=spec)
        ]
    )
    ...
    mock_orchestrator.run_workflow.return_value = Workflow(
        workflow_id="wf_1", 
        name="Test Workflow",
        steps=[WorkflowStep(step_id="s1", description="Test step", status=WorkflowStatus.COMPLETED, spec=spec)],
