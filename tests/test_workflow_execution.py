...
async def test_builder_and_orchestrator_integration():
    ...
    req = TaskRequest(
        request_id="req2", 
        user_input="Calculate mean of [10, 20, 30]"
    )
    ...
    if not workflow.steps[0].spec.args:
        workflow.steps[0].spec = workflow.steps[0].spec.model_copy(update={"args": {"data": [10, 20, 30], "operation": "descriptive"}})
...
async def test_multi_step_independent():
    ...
    # Inject args
    workflow.steps[0].spec = workflow.steps[0].spec.model_copy(update={"args": {"data": [1, 1, 1], "operation": "descriptive"}})
    workflow.steps[1].spec = workflow.steps[1].spec.model_copy(update={"args": {"data": [2, 2, 2], "operation": "descriptive"}})
