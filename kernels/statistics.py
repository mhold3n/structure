    ...
    def execute(self, input: KernelInput) -> KernelOutput:
        ...
        try:
            if method == "descriptive":
                if data is None:
                     return self._make_output(input.request_id, success=False, error="Missing data for descriptive stats")
                return self._make_output(input.request_id, success=True, result=self._descriptive(data))
            elif method == "ttest_ind":
                if data is None:
                     return self._make_output(input.request_id, success=False, error="Missing data for t-test")
                return self._make_output(input.request_id, success=True, result=self._ttest_ind(data))
            elif method == "regression":
                if data is None:
                     return self._make_output(input.request_id, success=False, error="Missing data for regression")
                return self._make_output(input.request_id, success=True, result=self._regression(data))
            elif method == "sample_size":
                 return self._make_output(input.request_id, success=True, result=self._sample_size(args))
            else:
                return self._make_output(input.request_id, success=False, error=f"Unknown operation '{method}'")
